-- Add missing template -> stream mappings. Existing mappings are byte-for-byte
-- identical to the currently deployed definition. Unknown templates still
-- return NULL (fail closed). No rows are touched; no queued email is released.
...
AS $function$
  SELECT CASE p_template
    ...
    WHEN 'team-approved' THEN 'transactional'
    WHEN 'team-rejected' THEN 'transactional'
    ...
    WHEN 'agent-activation-reminder' THEN 'transactional'
