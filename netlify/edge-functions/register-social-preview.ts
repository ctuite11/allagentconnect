...
export default async function handler(request: Request, context: any) {
  try {
    ...
    // Only intercept /register with listing_id param for crawlers
    const listingId = url.searchParams.get("listing_id");
    if (!listingId || !CRAWLER_REGEX.test(userAgent)) {
      return context.next(); // Let SPA handle it
    }
    
    // For crawlers with listing_id, proxy to Supabase social-preview function
    ...
    console.error("register-social-preview edge function error:", error);
