...
BEGIN
  ...
  LOOP
    ...
    END IF;
    IF sender_name IS NULL OR sender_name = '' THEN sender_name := 'Someone'; END IF;

    snippet := left(coalesce(r.body, ''), 500);

    SELECT EXISTS (SELECT 1 FROM public.agent_profiles ap WHERE ap.id = r.recipient_agent_id) INTO v_is_agent_recipient;
    ...
    IF r.listing_id IS NOT NULL THEN
      SELECT jsonb_build_object(
        ...
        'price', l.price, 'bedrooms', l.bedrooms, 'bathrooms', l.bathrooms,
        'square_feet', l.square_feet, 'status', l.status,
        'property_type', l.property_type, 'listing_type', l.listing_type,
        'neighborhood', l.neighborhood, 'listing_number', l.listing_number,
        'photos', l.photos, 'condo_details', l.condo_details,
        'property_styles', l.property_styles, 'description', l.description,
        'year_built', l.year_built, 'garage_spaces', l.garage_spaces, 'lot_size', l.lot_size
      ) INTO v_listing FROM public.listings l WHERE l.id = r.listing_id;

      IF v_listing IS NOT NULL THEN
        v_listing_url := CASE WHEN v_is_agent_recipient THEN '/property/' || r.listing_id::text ELSE '/consumer-property/' || r.listing_id::text END;
      END IF;
    END IF;
