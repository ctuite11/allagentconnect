-- Safe delete / unlink for agent-owned hot sheets that are still pending for a CRM buyer
-- (no accepted workspace invite, no accepted client_hotsheet_invite token for that buyer).
...
BEGIN
  ...
  -- Buyer has accepted workspace (linked AAC user): keep as persistent activity.
  IF EXISTS (
    SELECT 1
  ...
  -- Accepted hot sheet invite for this buyer + sheet.
  IF EXISTS (
    SELECT 1
