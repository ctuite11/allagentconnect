-- Phase 3: revoke anonymous direct SELECT on public.listings.
-- Public marketing reads remain via:
--   - get_public_listing / get_public_listing_agent (detail pages)
--   - listings_public view (browse/search/directory cards — marketing columns only)
-- No RPC contract changes. No sensitive columns exposed to anon.

BEGIN;

-- 1) Authenticated-only SELECT policy on the base table (same published-status set + account match).
...
-- 2) Table privilege: remove SELECT from anon (authenticated/service_role unchanged).
REVOKE SELECT ON TABLE public.listings FROM anon;

-- 3) Marketing-only public view for list/browse/directory surfaces.
--    Runs as owner (security_invoker off) so anon can read published marketing
--    columns without base-table SELECT. Omits lockbox/showing/commission/notes.
...
-- 4) Companion: showing_requests INSERT WITH CHECK previously used
--    EXISTS (SELECT 1 FROM listings …) under caller RLS. After anon loses
--    listings SELECT, guest Schedule Showing inserts would fail closed.
