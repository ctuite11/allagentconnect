-- =============================================================================
-- Agent Account Delegates — Scaffold → Approved Phase 1 reconciliation
--
-- Run when live DB has Lovable scaffold columns (owner_agent_id / member_user_id).
-- No-op when approved schema (owner_user_id / delegate_user_id) already exists.
--
-- After this: apply 20260704120000_agent_account_delegates_phase2_rls.sql only.
-- Keep agent_account_delegates = false.
-- =============================================================================
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'agent_account_members'
      AND column_name = 'owner_user_id'
  ) THEN
    RAISE NOTICE 'agent_account_delegates: approved schema already present; skipping scaffold reconciliation';
    RETURN;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'agent_account_delegates: no scaffold detected; approved Phase 1 migration will create schema';
    RETURN;
  END IF;

  RAISE NOTICE 'agent_account_delegates: removing Lovable scaffold schema';

  DROP POLICY IF EXISTS "aam_owner_select" ON public.agent_account_members;
  DROP POLICY IF EXISTS "aam_member_select" ON public.agent_account_members;
  DROP POLICY IF EXISTS "aam_owner_insert" ON public.agent_account_members;
  DROP POLICY IF EXISTS "aam_owner_update" ON public.agent_account_members;
  DROP POLICY IF EXISTS "aam_owner_delete" ON public.agent_account_members;
  DROP POLICY IF EXISTS "aac_owner_all" ON public.agent_active_context;
  ...
  DROP TABLE IF EXISTS public.agent_active_context CASCADE;
  DROP TABLE IF EXISTS public.agent_account_members CASCADE;

  DROP FUNCTION IF EXISTS public.matches_current_account(uuid);
  DROP FUNCTION IF EXISTS public.is_account_member_of(uuid, uuid);
  DROP FUNCTION IF EXISTS public.current_account_owner_id();
  DROP FUNCTION IF EXISTS public.delegates_enabled();
END $recon$;

-- ---------------------------------------------------------------------------
-- Approved Phase 1 foundation (idempotent; skipped when tables already exist)
-- ---------------------------------------------------------------------------
...
-- Drop obsolete scaffold helpers if they survived
DROP FUNCTION IF EXISTS public.delegates_enabled();
DROP FUNCTION IF EXISTS public.is_account_member_of(uuid, uuid);