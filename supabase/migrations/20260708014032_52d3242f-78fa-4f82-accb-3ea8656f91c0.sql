...
BEGIN
  ...
  -- cleaned up separately by _purge_expired_agent_active_context() and
  -- assert_agent_context() on write paths.
  ...
  IF v_ctx IS NOT NULL AND (v_expires IS NULL OR v_expires > now()) THEN
    ...
    ELSIF public.is_accepted_delegate_for(v_ctx) THEN
      RETURN v_ctx;
    END IF;
    -- Otherwise fall through and ignore invalid context (no delete on read).
  END IF;
