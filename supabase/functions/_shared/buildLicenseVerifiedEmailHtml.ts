/* ------------------------------------------------------------------ */
/*  License Verified — simplified confirmation email                    */
/* ------------------------------------------------------------------ */
...
export interface LicenseVerifiedEmailOptions {
  ...
  /** Overrides the CTA button label (activation flow uses "Activate Account"). */
  ctaLabel?: string;
...
export function buildLicenseVerifiedEmailHtml(opts: LicenseVerifiedEmailOptions): string {
  ...
  const ctaLabel = opts.ctaLabel?.trim() || "Activate Account";
  ...
  const greeting = `Hi ${escapeHtml(opts.agentName || "there")},`;
    ...
    <tr><td align="center" style="padding:32px 16px 48px;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;">
        ...
        <tr><td align="center" style="background-color:${NAVY};border-radius:14px 14px 0 0;padding:36px 40px 28px;">
          ...
          <h1 style="margin:0;font-size:26px;line-height:1.25;font-weight:700;letter-spacing:-0.02em;color:#ffffff;font-family:system-ui,-apple-system,'Segoe UI',Roboto,Arial,sans-serif;">Your account is ready</h1>
        </td></tr>

        <!-- Body -->
        <tr><td style="background-color:#ffffff;border:1px solid ${BORDER};border-top:none;padding:36px 40px 44px;">
          <p style="margin:0 0 6px;font-size:16px;line-height:1.6;color:${TEXT_DARK};font-family:system-ui,-apple-system,'Segoe UI',Roboto,Arial,sans-serif;">${greeting}</p>
          <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${TEXT_BODY};font-family:system-ui,-apple-system,'Segoe UI',Roboto,Arial,sans-serif;">Your real estate license has been verified. Activate your All Agent Connect account to get started.</p>
