...
// Vertical spread angles for left nodes (fan from top-left to bottom-left)
// Returns y-offset as fraction of half-height for each node
...
export const FeaturesOverviewSection = (): JSX.Element => {
  // SVG viewport: 1100 wide × 400 tall.  Hub sits at centre.
  const W = 1100;
  const H = 400;
  const cx = W / 2;
  const cy = H / 2;

  // Hub ring radii (visual concentric circles)
  const r1 = 36, r2 = 56, r3 = 80;

  // Node column x positions
  const leftNodeX  = 88;   // right edge of left icons column
  const rightNodeX = W - 88; // left edge of right icons column

  // Vertical spread
  const spread = 148; // half-height of fan — tighter

  const leftNodes = LEFT_NODES.map((n, i) => ({
    ...n,
    x: leftNodeX,
    y: cy + leftYFractions[i] * spread,
  }));

  const rightNodes = RIGHT_NODES.map((n, i) => ({
    ...n,
    x: rightNodeX,
    y: cy + rightYFractions[i] * spread,
  }));

  // Build a curved dashed path from a node to the hub edge (lines terminate closer to center)
  const buildPath = (nx: number, ny: number, side: "left" | "right") => {
    const isLeft = side === "left";
    // Hub edge point — terminate at r3 (outer ring)
    ...
    // Control points: shorter, more direct curve
    const cpx1 = isLeft ? npx + 70 : npx - 70;
    const cpy1 = npy;
    const cpx2 = isLeft ? hx - 50 : hx + 50;
    const cpy2 = hy;

    return `M ${npx} ${npy} C ${cpx1} ${cpy1}, ${cpx2} ${cpy2}, ${hx} ${hy}`;
  };
      ...
      <div className="max-w-[1200px] mx-auto flex flex-col items-center gap-16">
        ...
        <div className="flex flex-col items-center gap-5 text-center max-w-[680px]">
          <div className="inline-flex items-center gap-2 pl-3.5 pr-5 py-2 bg-[#2537ff0f] border border-[#2537ff26] rounded-full">
            <span className="[font-family:'Manrope',Helvetica] font-semibold text-[#2537ff] text-[14px] tracking-[0.28px]">⚡ Capability</span>
          </div>
          <h2 className="[font-family:'Manrope',Helvetica] font-bold text-[#292d32] text-[clamp(28px,4vw,52px)] tracking-[-2px] leading-[1.15]">
            Turning network intelligence into real results.
          </h2>
          <p className="[font-family:'Manrope',Helvetica] font-medium text-[#40424d] text-[17px] leading-[1.7]">
            Data in. Dollars out.
          </p>
        </div>
        ...
        <div className="hidden lg:block w-full -mt-4" style={{ maxWidth: W }}>
          <svg
            ...
            aria-label="All Agent Connect network diagram"
          >
            {/* ── Dashed connection lines (behind everything) ── */}
            {leftNodes.map((n) => (
              <path
                key={n.label + "-line"}
                d={buildPath(n.x, n.y, "left")}
                fill="none"
                stroke="#c7cfe0"
                strokeWidth="1.4"
                strokeDasharray="5 4"
            ...
            {/* ── Hub hazy glow rings (soft diffused, layered outward → inward) ── */}
            {/* Outermost soft glow blur layers */}
            ...
            {/* Hub monogram — AAC lettermark */}
            <g transform={`translate(${cx}, ${cy})`}>
              {/* "AAC" rendered as tight stacked letterform inside the blue disc */}
              {/* Top row: "AA" */}
              {/* Left A */}
            ...
            {leftNodes.map((n) => {
              ...
              return (
                <g key={n.label} transform={`translate(${startX}, ${n.y - pillH / 2})`}>
                  {/* Square icon box */}
                  <rect x="0" y="0" width={iconBoxSize} height={pillH} rx="6"
                    fill="white" stroke="#e8eaf0" strokeWidth="1.2"
                  ...
                  {/* Pill label */}
                  <rect x={iconBoxSize + gap} y="0" width={pillW} height={pillH} rx={pillH / 2}
                    fill="white" stroke="#2537ff" strokeWidth="1.2"
                    style={{ filter: "drop-shadow(0 1px 4px rgba(0,0,0,0.07))" }}
                  />
                  <text
                    x={iconBoxSize + gap + pillW / 2}
                    y={pillH / 2 + 4.5}
                    textAnchor="middle"
              ...
              return (
                <g key={n.label} transform={`translate(${n.x}, ${n.y - pillH / 2})`}>
                  ...
                  <rect x="0" y="0" width={pillW} height={pillH} rx={pillH / 2}
                    fill="white" stroke="#1bc572" strokeWidth="1.2"
                    style={{ filter: "drop-shadow(0 1px 4px rgba(0,0,0,0.07))" }}
                  />
                  ...
                  {/* Square icon box */}
                  <rect x={pillW + gap} y="0" width={iconBoxSize} height={pillH} rx="6"
                    fill="white" stroke="#e8eaf0" strokeWidth="1.2"
                    style={{ filter: "drop-shadow(0 1px 4px rgba(0,0,0,0.07))" }}
                  />
                  <g transform={`translate(${pillW + gap + iconBoxSize / 2 - 7}, ${pillH / 2 - 7})`}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#92a2bb" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                      <path d={n.icon}/>
                    </svg>
                  </g>
                </g>
        ...
        <div className="lg:hidden w-full flex flex-col gap-6">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="[font-family:'Manrope',Helvetica] font-bold text-[#292d32] text-[13px]">All Agent Connect</p>
            <p className="[font-family:'Manrope',Helvetica] font-medium text-[#92a2bb] text-[12px] -mt-2">Private agent network</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {LEFT_NODES.map((n) => (
              <div key={n.label} className="flex items-center gap-2.5 px-4 py-2.5 bg-white rounded-full border border-[#2537ff] shadow-sm">
                ...
                <span className="[font-family:'Manrope',Helvetica] font-semibold text-[#40424d] text-[12px]">{n.label}</span>
              </div>
            ))}
            {RIGHT_NODES.map((n) => (
