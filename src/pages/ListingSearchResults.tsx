...
const ListingSearchResults = () => {
  ...
  /** Map split: minimal full-width strip (navigation + count only). */
  const renderMapSplitMinimalHeader = () => (
    <div
      className="mb-3 flex shrink-0 items-center justify-between gap-3 border-b border-zinc-200/60 pb-2.5 pt-0.5"
    ...
    >
      <div className="flex min-w-0 items-center gap-1.5">
        <button
          ...
          className="-ml-0.5 shrink-0 rounded-md p-0.5 text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-900"
          aria-label="Go back"
        >
          <ArrowLeft className="h-4 w-4" strokeWidth={2} />
        </button>
        <h1 className="truncate text-xs font-semibold tracking-tight text-zinc-900">Edit Search</h1>
      </div>
      <span className="inline-flex shrink-0 items-center rounded-full border border-zinc-200/70 bg-zinc-50 px-2 py-0.5 text-[11px] font-medium tabular-nums text-zinc-600">
        {displayedListings.length} listings found
      </span>
    </div>
  );

  /** Map split: cards column only — row1 actions, row2 view+sort (aligned with map top). */
  const renderMapSplitCardsChrome = () => {
    const actionBtnClass =
      "h-[26px] shrink-0 whitespace-nowrap rounded-full border border-zinc-200/90 bg-white px-2 text-[11px] font-medium text-zinc-700 transition-colors hover:border-zinc-300 hover:bg-zinc-50";
    const actionIconClass = "mr-0.5 h-3 w-3 shrink-0 !text-[hsl(221,92%,51%)]";
    const shareTriggerClass =
      "h-[26px] shrink-0 whitespace-nowrap rounded-full border border-zinc-200/90 bg-white px-2 text-[11px] font-medium [&_svg]:mr-1 [&_svg]:!h-3 [&_svg]:!w-3";

    return (
      <>
        <div
          className="shrink-0 border-b border-zinc-100/80 px-1 py-1.5 sm:px-2"
          aria-label="Listing actions"
        >
          <div className="flex min-w-0 flex-wrap items-center gap-1.5 sm:justify-between">
            ...
            {selectedRows.size > 0 && (
              <span className="inline-flex shrink-0 items-center rounded-full border border-primary/20 bg-blue-50/50 px-2 py-px text-[11px] font-medium leading-none text-primary whitespace-nowrap">
        ...
        <div
          className="flex shrink-0 items-center justify-between gap-2 border-b border-zinc-100/80 px-1 py-1.5 sm:px-2"
          aria-label="View and sort"
        >
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] text-zinc-500">View</span>
            <div className="inline-flex rounded-full border border-zinc-200/80 bg-zinc-50/80 p-px">
              <button
                type="button"
                onClick={() => setResultsView("map")}
                className={cn(
                  "h-[22px] min-w-[2.5rem] rounded-full px-2 text-[11px] font-medium transition-colors",
                  resultsView === "map"
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Select value={sortSelectValue} onValueChange={handleSortSelect}>
              <SelectTrigger className="h-[26px] w-[116px] shrink-0 rounded-full border-zinc-200/90 bg-white px-2 text-[11px] focus:border-zinc-300 focus:outline-none focus:ring-0 focus:ring-offset-0">
                <SelectValue />
              </SelectTrigger>
      ...
      <main className="flex-1">
        <div className="max-w-[1400px] mx-auto">
          ...
          <section className="bg-transparent px-5 pb-6 pt-0">
            {showMapSplit ? (
              <>
                {renderMapSplitMinimalHeader()}
                <div className="flex min-h-[50vh] flex-col-reverse items-start gap-4 lg:grid lg:grid-cols-[minmax(0,40%)_minmax(0,60%)] lg:h-[calc(100dvh-7.5rem)] lg:min-h-0 lg:flex-none lg:gap-4">
                  <section className="w-full overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-[0_10px_26px_rgba(15,23,42,0.07)] h-[50dvh] min-h-0 sm:h-[54dvh] lg:h-full lg:min-h-0 lg:sticky lg:top-4 lg:self-start">
                  ...
                  <div className="flex min-h-[50vh] w-full min-w-0 flex-col lg:h-full lg:min-h-0">
                    {renderMapSplitCardsChrome()}
                    <section className="mt-1 flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-[0_10px_26px_rgba(15,23,42,0.07)]">
