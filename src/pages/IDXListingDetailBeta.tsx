...
async function fetchListingDetail(mlsNumber: string): Promise<RepliersListingsResponse> {
  const response = await fetch(
    `/api/repliers/listing-detail?mlsNumber=${encodeURIComponent(mlsNumber)}`,
    {
      method: "GET",
...
/**
 * IDX Listing Detail Beta page - single MLS listing view using Repliers API
 */
export default function IDXListingDetailBeta() {
  const { mlsNumber } = useParams<{ mlsNumber: string }>();
  ...
  const { data, isLoading, error } = useQuery<RepliersListingsResponse, Error>({
    ...
    queryFn: () => fetchListingDetail(mlsNumber!),
    enabled: !!mlsNumber,
    staleTime: 60 * 1000,
  });
