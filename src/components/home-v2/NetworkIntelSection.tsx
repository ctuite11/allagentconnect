...
const agentCards = [
  { name: "Sarah M.", role: "Listing Agent", location: "Boston, MA", img: property1 },
  { name: "David R.", role: "Buyer Agent", location: "Cambridge, MA", img: heroEditorial },
  { name: "Jennifer L.", role: "Listing Agent", location: "Brookline, MA", img: property2 },
  { name: "Michael K.", role: "Buyer Agent", location: "Newton, MA", img: property3 },
];

const recentActivity = [
  { type: "New Listing", agent: "Sarah M.", detail: "4BR Colonial — Wellesley", time: "2m ago" },
  { type: "Match", agent: "David R.", detail: "Buyer matched — $1.2M range", time: "8m ago" },
  { type: "Showing", agent: "Jennifer L.", detail: "Private showing confirmed", time: "15m ago" },
  { type: "Intel", agent: "Michael K.", detail: "Price reduction — Newton", time: "22m ago" },
];
    ...
    <section id="network" className="py-24 lg:py-32 bg-background">
      <div className="max-w-7xl mx-auto px-6 lg:px-16">
        ...
        <div className="mt-16 rounded-2xl bg-zinc-950 border border-zinc-800 overflow-hidden shadow-2xl">
          ...
          <div className="flex">
            {/* Sidebar nav */}
            <div className="hidden md:flex flex-col w-48 border-r border-zinc-800/60 p-4 gap-1">
              {["Dashboard", "Listings", "Matches", "Messages", "Network", "Analytics"].map((item, i) => (
                <div
                  key={item}
                  className={`px-3 py-2 rounded-lg text-xs font-medium ${i === 0 ? "bg-accent/10 text-accent" : "text-zinc-500 hover:text-zinc-300"}`}
                >
                  {item}
                </div>
              ))}
            ...
            <div className="flex-1 p-6 lg:p-8">
              ...
              <div className="grid lg:grid-cols-2 gap-4">
                ...
                <div className="bg-zinc-900/40 rounded-xl p-5 border border-zinc-800/40">
                  <span className="text-sm text-zinc-400 font-medium">Recent Activity</span>
                  <div className="mt-3 space-y-2.5">
                    {recentActivity.map((item, i) => (
                      <div key={i} className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-zinc-600 shrink-0 ml-2">{item.time}</span>
                      </div>
                    ))}
                  </div>
                </div>
