    ...
    <div className="flex min-h-0 w-full min-w-0 flex-1 flex-col">
      <div className="mb-2.5 flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-[12px] leading-snug text-neutral-500">Name, contact, hot sheets, favorites, notes</p>
        </div>
        <button
          type="button"
      ...
      <div className="max-h-[min(24rem,55vh)] overflow-y-auto overscroll-contain rounded-xl border border-zinc-100 bg-white md:max-h-[min(24rem,50vh)]">
        {buyers.length === 0 ? (
                  ...
                  <li key={b.id}>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {b.attentionNote ? (
                          <span className="shrink-0 rounded-full bg-amber-50 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-800 ring-1 ring-amber-200/80">
                            {b.attentionNote}
                          </span>
                        ) : null}
                      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-100 bg-zinc-50/50">
                    ...
                    <th className="hidden lg:table-cell px-2.5 py-1.5 text-left text-[11px] font-medium text-neutral-500">
                      Email
                    </th>
                    <th className="w-[4.5rem] px-1.5 py-1.5 text-center text-[11px] font-medium text-neutral-500">
                      Sheets
                    </th>
                    <th className="w-20 px-1.5 py-1.5 text-center text-[11px] font-medium text-neutral-500">
                      <span className="inline-flex items-center justify-center gap-0.5">
                        <Heart className="h-3 w-3 text-[#50C878]" aria-hidden strokeWidth={2} />
                        Fav
                      </span>
                    </th>
                    <th className="min-w-[6rem] px-2 py-1.5 text-left text-[11px] font-medium text-neutral-500">Note</th>
                ...
                <tbody className="divide-y divide-zinc-100">
                  {buyers.map((b) => (
                    <tr
                      ...
                      onClick={() => navigate(`/success-hub/buyers/${b.id}`)}
                    >
                      <td className="max-w-[8rem] truncate px-2.5 py-1.5 text-[13px] font-medium text-neutral-900">
                        {displayName(b)}
                      </td>
                      <td className="max-w-[5.5rem] whitespace-nowrap px-2.5 py-1.5 text-[12px] text-neutral-600">
                        {formatPhone(b.phone)}
                      </td>
                      <td className="hidden max-w-[11rem] truncate px-2.5 py-1.5 text-[12px] text-neutral-600 lg:table-cell">
                        {b.email || "—"}
                      </td>
                      ...
                      <td className="max-w-[10rem] px-2 py-1.5">
                        {b.attentionNote ? (
                          <span className="inline-flex max-w-full truncate rounded-full bg-amber-50 px-1.5 py-0.5 text-[10px] font-semibold text-amber-900 ring-1 ring-amber-100">
                            {b.attentionNote}
                          </span>
                        ) : (
