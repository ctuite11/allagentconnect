...
-- 1) Canonical Agent Network population
WITH network AS (
  SELECT user_id
  FROM public.get_verified_agent_ids()
),
-- 2) Corrected Communications base (= Network RPC)
comms_base AS (
  SELECT user_id
  FROM public.get_verified_agent_ids()
),
-- 3) Legacy independent Comms eligibility (pre-alignment)
legacy_comms AS (
  SELECT s.user_id
...
-- Detail: any drift between Network RPC and corrected Comms base (must be empty)
WITH network AS (
...
-- Detail: legacy-only extras (still visible under old photo/activation OR rule)
WITH network AS (
  SELECT user_id FROM public.get_verified_agent_ids()
),
legacy_comms AS (
