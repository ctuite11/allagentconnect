/**
 * Local, read-only Hot Sheet dry-run (zero writes / zero sends).
 *
 * Usage:
 *   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... \
 *     deno run --allow-net --allow-env scripts/dry-run-hot-sheet-listing.ts <listing_id>
 *
 * Does not insert email_jobs, does not invoke producers, does not send mail.
 */
...
} from "../supabase/functions/_shared/hotSheetAgentDelivery.ts";
import {
  getHotSheetStatusCopy,
  normalizeStatusKey,
} from "../supabase/functions/_shared/hotSheetStatusCopy.ts";

const listingId = Deno.args[0];
if (!listingId) {
  console.error("Usage: dry-run-hot-sheet-listing.ts <listing_id>");
  Deno.exit(1);
}
...
if (!listing) {
  console.error(JSON.stringify({ error: "listing not found", listing_id: listingId }));
  Deno.exit(2);
}
...
const { data: hotSheets, error: hsErr } = await supabase
  .from("hot_sheets")
  .select(
    "id, user_id, name, criteria, is_active, notify_agent_email, notification_schedule",
  )
  .eq("is_active", true)
...
for (const hs of hotSheets || []) {
  ...
  for (const row of clients || []) {
    ...
    const accepted =
      (cid && acceptedIds.has(cid)) || (email && acceptedEmails.has(email));
    if (!email) {
  ...
  const sheetReport = {
    ...
    excluded: { agent: exclusions, clients: clientExcluded },
